...
__global__ void __launch_bounds__(block_size) ctwc_a0_backward_kernel(
    ...
    for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx) {
        ...
        if (nidx < N) { // 有效的邻居索引
            ...
            // 权重相关的梯度计算
            const float grad_final_weight = grad_out.v[f_idx] * diff;
...
void ctwc_a0_backward(
    ...
    ctwc_a0_backward_kernel<block_size><<<grid_size, block_size>>>(
