...
__global__ void __launch_bounds__(block_size) ctwc_a0_forward_kernel(
    ...
    float *weight_store,   //      B N C 4    存储相关性权重 (为方向传播存储)
    ...
    for (uint64_t i = 0; i < k; ++i) {
        ...
        const float final_weight = __expf(-(xyz_dist * 0.1f + was_dist*0.1f + cos_dist*0.1f));
        // 读取邻居点特征并应用权重
...
void ctwc_a0_forward(
    ...
    ctwc_a0_forward_kernel<block_size>
