...
__global__ void __launch_bounds__(block_size) knn_spse_4_backward_kernel(
    float *wgrad,             //      A 16 C
    const float *grad,        //      B N C
    const uint32_t *back_idx, //      B N C
    const float *kxyz,        //      B N 4
    const float *weight,      //      16 C
    const uint64_t N,
    const uint64_t C,
    const uint64_t BNC
){
    ...
    // fix bias
    const float4 center = *(float4*)(kxyz + bNn*8);
    const float p9 = proj[9], p10 = proj[10], p11 = proj[11];
    {
        const float x = center.x + proj[9],     //三轴平移量
                    y = center.y + proj[10],
                    z = center.z + proj[11];
        proj[9] = -(x*proj[0] + y*proj[3] + z*proj[6]);     //三轴旋转
        proj[10] = -(x*proj[1] + y*proj[4] + z*proj[7]);
        proj[11] = -(x*proj[2] + y*proj[5] + z*proj[8]);
    }
    // backward min_dist
    float proj_grad[12];
    const float min_dist_grad = grad[idx] * 2;
    {
        const float4 nf = *(float4*)(kxyz + b*N*8 + back_idx[idx]*8 + 4);       //后4个邻域特征，r，g，b，h
    ...
    {
        const float4 xyz = *(float4*)(kxyz + b*N*8 + back_idx[idx]*8);          //前4个特在: x,y,z,0
        const float x = proj[9] + xyz.x * proj[0] + xyz.y * proj[3] + xyz.z * proj[6],  //坐标平移+旋转
...
void knn_spse_4_backward(
    ...
    knn_spse_4_backward_kernel<block_size>
