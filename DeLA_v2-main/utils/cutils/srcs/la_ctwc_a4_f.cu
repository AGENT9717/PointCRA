...
__global__ void __launch_bounds__(block_size) ctwc_a4_forward_kernel(
    ...
    for (uint64_t i = 0; i < k; ++i) {
        ...
        float temp = (1.0f-(similarity + 1.0f) * 0.5f);
        const float cos_dist = temp*temp+0.001f;  // 从[-1,1]映射到[0,1]
...
void ctwc_a4_forward(
    ...
    ctwc_a4_forward_kernel<block_size>
