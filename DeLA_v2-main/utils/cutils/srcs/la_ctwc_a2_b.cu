...
__global__ void __launch_bounds__(block_size) ctwc_a2_backward_kernel(
    ...
    for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx) {
        ...
        const float was_dist = stored_weights.v[f_idx*4+1];
        const float cos_dist = stored_weights.v[f_idx*4+2];
        ...
        if (nidx < N) { // 有效的邻居索引
            ...
            // 计算权重对各个距离的偏导
            const float d_final_weight_d_was = -grad_final_weight * was_dist;
            const float d_final_weight_d_cos = -grad_final_weight * cos_dist;
...
void ctwc_a2_backward(
    ...
    ctwc_a2_backward_kernel<block_size><<<grid_size, block_size>>>(
