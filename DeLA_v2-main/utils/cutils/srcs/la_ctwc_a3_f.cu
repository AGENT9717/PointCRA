...
__global__ void __launch_bounds__(block_size) ctwc_a3_forward_kernel(
    ...
    for (uint64_t i = 0; i < k; ++i) {
        ...
        const float cos_dist = (1.0f-(similarity + 1.0f)*0.5f)*scale;  // 从[-1,1]映射到[2,0]
        const float global_dist = was_dist*l_weight.x;
        const float local_dist = cos_dist*l_weight.y;
        const float final_weight = __expf(-( local_dist*gate+ global_dist*(1.0f-gate)));
        // 读取邻居点特征并应用权重
        ...
        for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx) {
            ...
            if (diff > max_val.v[f_idx]) {
                ...
                max_weight.v[f_idx*4+1]= was_dist;
                max_weight.v[f_idx*4+2]= cos_dist;
            }
        }
    }
...
void ctwc_a3_forward(
    ...
    float gate_val = gate.item<float>();
    ctwc_a3_forward_kernel<block_size>
