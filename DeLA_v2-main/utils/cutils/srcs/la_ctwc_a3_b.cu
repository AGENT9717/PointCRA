...
__global__ void __launch_bounds__(block_size) ctwc_a3_backward_kernel(
    ...
    for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx) {
        ...
        const float was_dist = stored_weights.v[f_idx*4+1];   //s1*d1*gate
        const float cos_dist = stored_weights.v[f_idx*4+2];  //s2*d2*(1-gate)
        ...
        if (nidx < N) { // 有效的邻居索引
            ...
            //grad_in_center.v[f_idx] -= grad_out.v[f_idx] * final_weight;
            ...
            // 计算权重对各个距离的偏导
            const float d_final_weight_d_was = -grad_final_weight * was_dist*(1.0f-gate);
            const float d_final_weight_d_cos = -grad_final_weight * cos_dist*gate;

            const float d_final_gate = grad_final_weight*(-l_weight.y*cos_dist+l_weight.x*was_dist);
            // 累积权重梯度
            grad_weight_local.x +=  d_final_weight_d_was;
            grad_weight_local.y +=  d_final_weight_d_cos;
            grad_gate_local += d_final_gate;
    ...
    atomicAdd(grad_gate, grad_gate_local);
...
void ctwc_a3_backward(
    ...
    const float gate_val = gate.item<float>();
    // 初始化梯度为0
    grad_input.zero_();
    grad_weight.zero_();
    grad_gate.zero_();
    ...
    ctwc_a3_backward_kernel<block_size><<<grid_size, block_size>>>(
