...
__global__ void __launch_bounds__(block_size) la_spse_a4_forward_kernel(
    ...
    // fix bias
    {
        ...
        proj[0] += center.x;    // 投影点+中心坐标
        proj[1] += center.y;
        proj[2] += center.z;
    }
    // pool
    const floatn cf = *(floatn*)(input + f_base + n*C); //中心点特征
    floatn max_val{-1e8, -1e8, -1e8, -1e8};
    uint32_t max_idx[group_size];
    for (uint64_t i=0; i<k; ++i)
    {
        const uint64_t nidx = nbr_idx[n_base+i];
        const floatn valn = *(floatn*)(input + f_base + nidx*C);   //邻域点特征
        const float dist = calc_distance(*(float4*)(xyz + b*N*4 + nidx*4), proj) * proj[3]; //计算邻域点到投影点的欧式距离并乘以缩放
        // const float mul = __expf(-dist) * (1.0f+dist);
        const float mul = __expf(-dist);    //距离越近权重越大，距离越远权重越小
        // const float mul = __fdividef(1.0f, 1.0f + dist);
        // const float mul = rsqrtf(1.0f + dist);
        for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx)
        {
            const float val = (valn.v[f_idx] - cf.v[f_idx]) * mul;  //当前邻域点的通道值-中心点特征的通道值
