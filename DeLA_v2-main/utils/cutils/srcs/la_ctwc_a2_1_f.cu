...
__global__ void __launch_bounds__(block_size) ctwc_a2_1_forward_kernel(
    ...
    for (uint64_t i = 0; i < k; ++i) {
        ...
        const float cos_dist = (1.0f-similarity)*scale;


        const float final_weight = __expf(-(was_dist*l_weight.x + cos_dist*l_weight.y));
        // 读取邻居点特征并应用权重
...
void ctwc_a2_1_forward(
    ...
    ctwc_a2_1_forward_kernel<block_size>
