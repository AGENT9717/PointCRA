...
__global__ void __launch_bounds__(block_size) ctwc_a1_backward_kernel(
    ...
    for (uint64_t f_idx = 0; f_idx < group_size; ++f_idx) {
        ...
        if (nidx < N) { // 有效的邻居索引
            ...
            // 计算权重对各个距离的偏导
            const float d_final_weight_d_xyz = -grad_final_weight * xyz_dist;
...
void ctwc_a1_backward(
    ...
    ctwc_a1_backward_kernel<block_size><<<grid_size, block_size>>>(
