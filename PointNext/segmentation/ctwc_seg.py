...
class CtwcSeg(nn.Module):
    ...
    def forward(self, data):
        if self.training:
            p, f,c_loss = self.encoder.forward_seg_feat(data)
...
@MODELS.register_module()
class CtwcVariableSegHead(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
...
@MODELS.register_module()
class CtwcMultiSegHead(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
