...
// __device__ __forceinline__ float calc_pearson_similarity(
//     const float* seq1,
//     const float* seq2,
//     const int length
// ) {
...
//     for (int i = 0; i < length; ++i) {
//         float d1 = seq1[i] - mean1;
//         float d2 = seq2[i] - mean2;
//         cov += d1 * d2;
//         var1 += d1 * d1;
//         var2 += d2 * d2;
//     }
//
//     // 3. 计算相关系数
//     float r = cov / (sqrtf(var1) * sqrtf(var2) + 1e-8f);
//
//     // 4. 映射到[0, 1]范围（CTWC需要）
//     return (r + 1.0f) * 0.5f;
// }
...
__device__ __forceinline__ float calc_spherical_similarity(
    ...
    for (int i = 0; i < length - 1; ++i) {
        ...
        float vec1_y = (seq1[i + 1] - seq1[i])*scale;  // 值变化

        float vec2_x = 1.0f;
        float vec2_y = (seq2[i + 1] - seq2[i])*scale;
...
// __device__ __forceinline__ float calc_direction_aware_euclidean(
//     const float* seq1,
//     const float* seq2,
//     const int length
// ) {
//     float dist = 0.0f;
//     float dir_penalty = 0.0f;
//
//     for (int i = 0; i < length - 1; ++i) {
//         float d1 = seq1[i+1] - seq1[i];
//         float d2 = seq2[i+1] - seq2[i];
//
//         // 幅度差异
//         float mag_diff = d1 - d2;
//         dist += mag_diff * mag_diff;
//
//         // 方向惩罚：方向相反时额外增加距离
//         if (d1 * d2 < 0) {
//             dir_penalty += fabsf(d1) + fabsf(d2);
//         }
//     }
//
//     // 加上幅度差异
//     for (int i = 0; i < length; ++i) {
//         float val_diff = seq1[i] - seq2[i];
//         dist += val_diff * val_diff * 0.1f;  // 值差异权重小一些
//     }
//
//     dist = sqrtf(dist + dir_penalty);
//     return 1.0f / (1.0f + dist / length);
// }

template <uint64_t block_size>
__global__ void __launch_bounds__(block_size) ctwc_b11_forward_kernel(
    const float *trend_seq,   //      B N T C_    趋势序列 (新增参数)
    ...
    const uint64_t c_ = idx % C_,   // 当前通道组索引
                   ...
                   n = bNn % N,     //当前批次点数
                   b = bNn / N,     //当前批次
                   trend_base_c = b * N * T * C_+ n * T * C_,  // 趋势序列基址 通道连续
                   n_base = bNn * k;

    //初始化邻域均值
    float channel_group_trend_mean[MAX_T];
    for (uint64_t t = 0; t < T; ++t) {
    channel_group_trend_mean[t] = 0;
    }
    //累加邻域均值
    for (uint64_t i = 0; i < k; ++i) {
        const uint64_t nidx = nbr_idx[n_base + i];
        const uint64_t neighbor_trend_base = b * N * C_ * T + nidx * C_ * T;
        for (uint64_t t = 0; t < T; ++t) {
            const float trend_val = trend_seq[neighbor_trend_base + c_ + t * C_];
            //channel_group_trend_neighbor[t] = trend_val-channel_group_trend_center[t];
            channel_group_trend_mean[t] += trend_val;
        }
    }
    //计算邻域均值
    for (uint64_t t = 0; t < T; ++t) {
    channel_group_trend_mean[t] = channel_group_trend_mean[t]/k;
    }
    ...
    for (uint64_t t = 0; t < T; ++t) {
        // 读取当前时间步的4个通道值
        const float trend_val = trend_seq[trend_base_c + t * C_ + c_ ];
        // 对4个通道求平均，得到该时间步的趋势值
        channel_group_trend_center[t] = trend_val-channel_group_trend_mean[t];
    }
    ...
    for (uint64_t i = 0; i < k; ++i) {
        ...
        for (uint64_t t = 0; t < T; ++t) {
            const float trend_val = trend_seq[neighbor_trend_base + c_ + t * C_];
            channel_group_trend_neighbor[t] = trend_val-channel_group_trend_mean[t];
            //channel_group_trend_neighbor[t] = trend_val-neighbor_mean;
        }
...
//         float temp = (1.0f-(similarity + 1.0f)*0.5f);  // 从[-1,1]映射到[2,0]
        const float similarity = calc_spherical_similarity(channel_group_trend_center, channel_group_trend_neighbor, T,scale);
        //const float similarity = calc_pearson_similarity(channel_group_trend_center, channel_group_trend_neighbor, T);
        //const float similarity = calc_direction_aware_euclidean(channel_group_trend_center, channel_group_trend_neighbor, T);
        float temp =  similarity ;
...
void ctwc_b11_forward(
    ...
    ctwc_b11_forward_kernel<block_size>
